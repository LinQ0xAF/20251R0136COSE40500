...
void TC5_setup(){
	...
	TC5->COUNT16.CC[0].reg = 7812;  // 1초 주기
...
void EIC_setup() {
	...
	while (EIC->STATUS.bit.SYNCBUSY); // 반드시 wait
}
...
void EIC_Handler(void)
{
	...
	if		(Num_EIC_interrupts == 1) {
		...
		Distance = distance;			//main에서 distance로 속도를 조절할 수 있도록 글로벌 변수에 저장
...
void TC5_Handler(void) {
	...
	// PA17(Trigger)로 10us pulse 생성
	PORT->Group[0].OUTSET.reg = (1 << 17);
	for (int i = 0; i < 10; i++);                // 10us 유지
